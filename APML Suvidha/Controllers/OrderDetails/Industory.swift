...
class Industory: UIViewController{
    
    //MARK: - Properties
  
    @IBOutlet weak var countryAboveTxtField: UITextField!
    @IBOutlet weak var stateCodeTxtField: UITextField!
    @IBOutlet weak var panTxtField: UITextField!
    ...
    func setUpLayout(){
        ...
        self.companyNameTxtField.text = model.CompanyName
         self.panTxtField.text = model.Pan
         self.stateCodeTxtField.text = model.StateCode
        self.countryAboveTxtField.text = model.Country
    }
    func getBillDetails() {
        WebServices.shared.getBilling(methodName: AppConstants.METHOD_GET_Billing_Details, Uniq: AppUserDefaults.value(forKey: .UNIQUE, fallBackValue: "").string!, Type: "2",completion: {(response,error) in
            if error == nil{
    ...
    func setBillDetails() {
        let model: BillingModel
        model = billModels[0]
        WebServices.shared.setBilling(methodName: AppConstants.METHOD_SET_Billing_Details, Uniq: AppUserDefaults.value(forKey: .UNIQUE, fallBackValue: "").string!, Type: "2", FirstName: self.firstNameTxtField.text!, LastName: lastNameTxtField.text!, Email: emailTxtField.text!, Mobile: mobileTxtField.text!, AlterMobile: alternateTxtField.text!, City: cityTxtField.text!, ZipCode: zipCodeTxtField.text!, State: stateTxtField.text!, Country: countryTxtField.text!, Address: addressTxtField.text!, Gst: gstnoTxtField.text!, Pan:panTxtField.text!, StateCode: stateCodeTxtField.text!, TaxState: model.TaxState, completion: {(response,error) in
            if error == nil{
                let status = response![AppConstants.STATUSTXT] as! Int
                let statusStr = String(status)
                switch(statusStr){
    ...
    @IBAction func submitClickedBtn(_ sender: Any) {
        ...
        guard countryAboveTxtField.text != ""  else { showToast(message: "Enter Country"); return }
        setBillDetails()
    }
